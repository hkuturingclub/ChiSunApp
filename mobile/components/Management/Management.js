...
const Management = ({ managementQuery }) => {
  ...
  const otherTeam = management.filter(item => item.rank > TUTOR);

  return (
    <Container>
      <Content padder>
        <Header title="Management Team" align="center" />
        <ManagementList items={seniorTeam} numColumns={1} seniorTeam />
        <Header title="Tutorial Team" align="center" />
        <ManagementList items={tutorialTeam} numColumns={2} showFloors />
        <ManagementList items={otherTeam} numColumns={2} />
      </Content>
    </Container>
  );
};
...
export default graphql(MANAGEMENT_QUERY, { name: "managementQuery" })(
  Management
);
